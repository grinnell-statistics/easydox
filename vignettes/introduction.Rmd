...
output: rmarkdown::render("introduction.Rmd", output_format = "html_vignette", output_file = "introduction.html")

# Output created: Introduction.html
...
```{r, message = FALSE}
dox_main(LogStrength ~ Brand + Water, Towels2)
```
...
```{r}
dox_inter(LogStrength ~ Brand + Water, Towels2)
```
...
```{r}
dox_main(Score~Major+Wordlist+Distracter, Memory)
```

```{r}
dox_inter(Score~Wordlist+Distracter, Memory)
```
