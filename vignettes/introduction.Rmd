...
```{r}
Towels_copy = dox_resid(LogStrength~Brand*Water, Towels2)
```
...
```{r}
dox_inter(LogStrength ~ Water + Brand, Towels2, label="Mean",text_size = 14)
```

```{r}
dox_inter(LogStrength ~ Water + Brand, Towels2, label="Effect", text_size = 14)
```
